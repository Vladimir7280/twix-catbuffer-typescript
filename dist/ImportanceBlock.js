...
exports.ImportanceBlock = void 0;
...
class ImportanceBlock {
    ...
    static deserialize(payload) {
        ...
        const votingEligibleAccountsCount = Utils_1.Utils.bufferToUint32(Uint8Array.from(byteArray));
        byteArray.splice(0, 4);
        const harvestingEligibleAccountsCount = Utils_1.Utils.bufferToBigInt(Uint8Array.from(byteArray));
        byteArray.splice(0, 8);
        const totalVotingBalance = Amount_1.Amount.deserialize(Uint8Array.from(byteArray));
        byteArray.splice(0, totalVotingBalance.size);
        const previousImportanceBlockHash = Hash256_1.Hash256.deserialize(Uint8Array.from(byteArray));
        byteArray.splice(0, previousImportanceBlockHash.size);
        const transactions = Utils_1.Utils.deserializeRemaining(Transaction_1.Transaction.deserialize, Uint8Array.from(byteArray), byteArray.length, 0);
        byteArray.splice(0, transactions.reduce((sum, c) => sum + Utils_1.Utils.getSizeWithPadding(c.size, 0), 0));
        return new ImportanceBlock({
            signature: signature,
    ...
    get size() {
        ...
        size += this.transactions.reduce((sum, c) => sum + Utils_1.Utils.getSizeWithPadding(c.size, 0), 0);
        return size;
    }
    serialize() {
        ...
        const votingEligibleAccountsCountBytes = Utils_1.Utils.uint32ToBuffer(this.votingEligibleAccountsCount);
        newArray = Utils_1.Utils.concatTypedArrays(newArray, votingEligibleAccountsCountBytes);
        const harvestingEligibleAccountsCountBytes = Utils_1.Utils.bigIntToBuffer(this.harvestingEligibleAccountsCount);
        newArray = Utils_1.Utils.concatTypedArrays(newArray, harvestingEligibleAccountsCountBytes);
        const totalVotingBalanceBytes = this.totalVotingBalance.serialize();
        newArray = Utils_1.Utils.concatTypedArrays(newArray, totalVotingBalanceBytes);
        const previousImportanceBlockHashBytes = this.previousImportanceBlockHash.serialize();
        newArray = Utils_1.Utils.concatTypedArrays(newArray, previousImportanceBlockHashBytes);
        const transactionsBytes = Utils_1.Utils.writeList(this.transactions, 0);
        newArray = Utils_1.Utils.concatTypedArrays(newArray, transactionsBytes);
        return newArray;
    }
}
exports.ImportanceBlock = ImportanceBlock;
//# sourceMappingURL=ImportanceBlock.js.map