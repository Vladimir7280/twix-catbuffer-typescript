...
class Utils {
    ...
    static deserialize(deserialize, payload, count) {
    ...
    static deserializeEnums(payload, count, itemSize) {
        const byteArray = Array.from(payload);
        const values = [];
        for (let i = 0; i < Utils.compact(count); i++) {
    ...
    static deserializeRemaining(deserialize, payload, payloadSize, alignment) {
        const byteArray = Array.from(payload);
